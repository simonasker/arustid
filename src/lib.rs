...
fn render_to_window(config: Config) -> Result<(), Box<Error>> {
    ...
    let window = video_subsystem
        .window("arustid", width as u32, height as u32)
        .position_centered()
        .opengl()
        .build()?;
...
fn render_to_image(config: Config) -> Result<(), Box<Error>> {
    ...
    config
        .output_filename
        .map_or(
            Err(From::from("No filename")),
            |f| surface.save(Path::new(&f)),
        )?;

    Ok(())
}

pub fn calculate_dimensions(sequence: &str,
                            length: i32,
                            angle: i32)
                            -> Result<(i32, i32, i32, i32), Box<Error>> {
